...
最近のパソコンは 64bit と考えられるが,
不安であれば 32bit しておけばよい.
...
```{r}
knitr::include_graphics("figs/rstudio-general.PNG")
```
...
```{r}
knitr::include_graphics("figs/rstudio-utf8.PNG")
```

あと, [R Markdown] で 真ん中あたりの [Show output preview in:] を `View Pane` に変更する

```{r}
knitr::include_graphics("figs/rstudio-rmd.PNG")
```
...
```{r}
knitr::include_graphics("figs/rstudio_project.gif")
```
...
```{r}
knitr::include_graphics("figs/rstudio_file.gif")
```
...
```{r}
knitr::include_graphics("figs/rstudio_help.gif")
```
...
```{r}
knitr::include_graphics("figs/rstudio_knit.gif")
```
