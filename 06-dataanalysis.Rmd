...
もう一つ教育年数を説明変数, 賃金を対数としたデータの回帰分析を示す. 
```{r}
data("CPS1985", package="AER")
summary(CPS1985)
...
library(mosaic)
fn <- makeFun(fm1)
CPS1985 %>% gf_point(log(wage)~education, color=~gender) %>% 
...
CPS1985 %>% gf_point(log(wage)~education, color=~gender) %>% 
  gf_fun(log(fn(education,gender="male"))~education,color="red") %>%
  gf_fun(log(fn(education,gender="female"))~education,color="green")
```

以下のようにして, 簡単に作図できる.
```{r}
CPS1985 %>% gf_point(log(wage)~education, color = ~gender) %>% gf_lm()
```
